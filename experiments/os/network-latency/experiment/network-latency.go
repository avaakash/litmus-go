...
import (
	...
	litmusLIB "github.com/litmuschaos/litmus-go/chaoslib/litmus/os-network-chaos/lib"
	clients "github.com/litmuschaos/litmus-go/pkg/clients"
	"github.com/litmuschaos/litmus-go/pkg/events"
	"github.com/litmuschaos/litmus-go/pkg/log"
	"github.com/litmuschaos/litmus-go/pkg/machine/common/connections"
	"github.com/litmuschaos/litmus-go/pkg/machine/network"
	experimentEnv "github.com/litmuschaos/litmus-go/pkg/os/network-chaos/environment"
...
// OSNetworkLatency contains steps to inject chaos
func OSNetworkLatency(clients clients.ClientSets) {
	...
	experimentEnv.GetENV(&experimentsDetails, "os-network-latency")
	...
	log.InfoWithValues("[Info]: Network loss parameters are ", logrus.Fields{
		"Network Interface": experimentsDetails.NetworkInterfaces,
		"Loss Percentage":   experimentsDetails.NetworkPacketLossPercentage,
		"Destination IPs":   experimentsDetails.DestinationIPs,
		"Destination Hosts": experimentsDetails.DestinationHosts,
	})
	...
	log.Info("[Status]: Verify that tc is available in the target machine")
	if err := network.CheckPrerequisites(chaosDetails.WebsocketConnections); err != nil {
		log.Errorf("Error occured during network loss steady-state validation, err: %v", err)
		failStep := "[pre-chaos]: Failed to verify if tc is present, err: " + err.Error()
		result.RecordAfterFailure(&chaosDetails, &resultDetails, failStep, clients, &eventsDetails)
		return
	}
	...
	case "litmus":
		if err := litmusLIB.PrepareOSNetworkChaos(&experimentsDetails, clients, &resultDetails, &eventsDetails, &chaosDetails); err != nil {
